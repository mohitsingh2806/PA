...
class NetEEPROMClass : NewEEPROMClass {
	...
	private:
		 ...
		 * Password functions
		 */
		/** Write password signature to enable reading of password from newEEPROM */
		void writePassSig();
	...
	public:
		...
		/** Write the full network settings to the newEEPROM and set the network
		 ...
		 *  @return True if network settings have been set in the newEEPROM. */
		bool netSigIsSet(void);
		/** Read the MAC address from newEEPROM.
		...
		*  @return IPAddress object with the Subnet Mask. */
		...
		/** Write the port value to newEEPROM.
		 ...
		 * @return True if there is a port specified in newEEPROM. */
		bool portSigIsSet(void);
		/** Read the port from newEEPROM */
		word readPort(void);
		...
		/** Write the password to the newEEPROM.
		...
		/** Read the password from newEEPROM
		 *  @return String object with the password. */
		String readPass(void);
		/** Print password stored in newEEPROM to serial. Serial has to be initalized in the sketch
		 *  @param serial pointer to the initialized serial (use &Serial) */
